...
void on_button_clicked_add(GtkWidget *widget, gpointer data) {
    int i = 0;
    int ret = 0;
    SPSR_INPUT_ST *obj = 0;
...
void on_button_clicked_close(GtkWidget *widget, gpointer data) {
    int ret = 0;
    const char *portname = gtk_entry_get_text(GTK_ENTRY(entries[1])); 
    spllog(0, "Button %s clicked!\n", (char *)data);
    ret = spsr_inst_close((char*)portname);
}
void on_button_clicked_write(GtkWidget *widget, gpointer data) {
    int ret = 0;
    int i , n;
...
int main(int argc, char *argv[]) {
    ...
    do {
        ...
        for (i = 0; i < 5; i++) {
            ...
            if(i == 0) {
                g_signal_connect(buttons[i], "clicked", G_CALLBACK(on_button_clicked_add), labels[i]);
            }
            else if(i == 1) {
                g_signal_connect(buttons[i], "clicked", G_CALLBACK(on_button_clicked_close), labels[i]);
            }
            else if(i == 2) {
                g_signal_connect(buttons[i], "clicked", G_CALLBACK(on_button_clicked_write), labels[i]);
            }
            else if(i == 3) {
                //g_signal_connect(buttons[i], "clicked", G_CALLBACK(on_button_clicked_close), labels[i]);
            }
