...
on_button_clicked_write(GtkWidget *widget, gpointer data)
{
	...
	if (myusleep > 0) {
		...
		ret = spsr_inst_write(portname, text_data, sizeof(text_data));
	}
...
main(int argc, char *argv[])
{
	...
	do {
		...
		gtk_window_set_title(
		    GTK_WINDOW(window), "GTK 7 Buttons & 7 Textboxes");
		// gtk_window_set_default_size(GTK_WINDOW(window), 400, 300);
		gtk_window_maximize(
		    GTK_WINDOW(window)); // Title bar and buttons stay visible
		g_signal_connect(
		...
		char labels[7][100] = {"ADD COM port", "REM COM port",
		    "WRITE COM port", "written data", "read data", "6", "7"};

		for (i = 0; i < 5; i++) {
			buttons[i] = gtk_button_new_with_label(labels[i]);
			if (i == 0) {
				g_signal_connect(buttons[i], "clicked",
				    G_CALLBACK(on_button_clicked_add),
				    labels[i]);
			} else if (i == 1) {
				g_signal_connect(buttons[i], "clicked",
				    G_CALLBACK(on_button_clicked_close),
				    labels[i]);
			} else if (i == 2) {
				g_signal_connect(buttons[i], "clicked",
				    G_CALLBACK(on_button_clicked_write),
				    labels[i]);
			} else if (i == 3) {
				// g_signal_connect(buttons[i], "clicked",
				// G_CALLBACK(on_button_clicked_close),
			...
			if (i < 4) {
				gtk_entry_set_text(
				    GTK_ENTRY(entries[i]), "/dev/ttyUSB0");
			} else {
				gtk_widget_set_size_request(entries[i], 1600,
				    300); // Width: 300px, Height: 30px
			}
		}
		...
		do {
			...
			if (evt->type == SPSR_EVENT_WRITE_ERROR) {
				...
				snprintf(text, GTK_TEST_BUF,
				    ...
				    "datalen: %d",
				    spsr_err_txt(evt->err_code), realdata,
				    datalen);
				break;
			}
			...
			if (evt->type == SPSR_EVENT_OPEN_DEVICE_ERROR) {
				/* Port name .*/
				snprintf(text, GTK_TEST_BUF,
				    "SPSR_EVENT_OPEN_DEVICE_ERROR|%s, "
				    "realdata: %s, datalen: %d",
				    spsr_err_txt(evt->err_code), realdata,
				    datalen);
				break;
			...
			if (evt->type == SPSR_EVENT_CLOSE_DEVICE_ERROR) {
				/* Port name .*/
				snprintf(text, GTK_TEST_BUF,
				    "SPSR_EVENT_CLOSE_DEVICE_ERROR|%s, "
				    "realdata: %s, datalen: %d",
				    spsr_err_txt(evt->err_code), realdata,
				    datalen);
				break;
...
spsr_call_back_read(void *data)
{
	...
	spllog(SPL_LOG_DEBUG,
	    "evt->total: %d, evt->pc: %d, evt->pl: %d, data: %s", evt->total,
	    evt->pc, evt->pl, evt->data + evt->pc);
